...
export class Themes implements OnInit {
  ...
  showSubThemesCount = true;
  showCreatedAt = false;
  showUpdatedAt = false;
  showUpdatedBy = false;

  private readonly STORAGE_KEY = 'b12.themes.columns.v2';
  ...
  constructor(private api: ThemeApi, private router: Router) {}
  ...
  load(): void {
    ...
    this.api.findAll().subscribe({
      ...
      error: (err) => {
        console.error(err);
        this.error = err?.error?.message || 'Impossible de charger les thématiques.';
        this.loading = false;
      },
    });
  }
  ...
  setSort(key: SortKey) {
    if (
      ...
      (key === 'subThemesCount' && !this.showSubThemesCount) ||
  ...
  toggleAll(checked: boolean) {
    ...
    if (checked) {
      for (const x of this.items) {
        if ((x as any).id != null) this.selected.add((x as any).id);
      }
    }
  }
  ...
  async runAction(): Promise<void> {
    ...
    // ✅ DUPLICATE : 1 seule ligne
    if (this.action === 'duplicate') {
      if (this.selected.size !== 1) {
        this.toast.warning('Veuillez sélectionner exactement 1 thématique à dupliquer.');
        return;
      }
      const id = Array.from(this.selected.values())[0];
      this.router.navigate(['/thematics/new'], { queryParams: { cloneId: id } });
      return;
    }
    ...
    if (this.action === 'delete') {
      const rows = this.allItems.filter(t => ids.includes((t as any).id));
      let msg = `Supprimer ${ids.length} thématique(s) ?\n\n`;
      for (const t of rows) {
        const count = (t as any).subThemesCount ?? (t as any).subThemes?.length ?? 0;
        msg += `• ${(t as any).name}`;
        if (count > 0) msg += ` — ${count} sous-thème(s)`;
        msg += '\n';
      }
  ...
  private handleExport(format: 'csv' | 'xlsx' | 'pdf', selected: boolean, includeArchived = false) {
    ...
    request.subscribe({
      next: (blob) => {
        const filename = selected ? `themes-selection.${format}` : `themes.${format}`;
  ...
  private applyLocalFilters() {
    ...
    if (q) {
      arr = arr.filter(
        (x) =>
          ((x as any).name ?? '').toLowerCase().includes(q) ||
          ((x as any).description ?? '').toLowerCase().includes(q)
      );
    }
    ...
    this.items = arr.filter((x) => !(x as any).archived);
    this.archivedItems = arr.filter((x) => !!(x as any).archived);
    this.updatePaged();
  }

  private sortValue(x: Theme): string | number {
    switch (this.sortKey) {
      case 'name':
        return ((x as any).name ?? '').toLowerCase();
      case 'description':
        return ((x as any).description ?? '').toLowerCase();
      case 'subThemesCount':
        return (x as any).subThemesCount ?? ((x as any).subThemes?.length ?? 0);
      case 'archived':
        return (x as any).archived ? 1 : 0;
      case 'createdAt':
        return (x as any).createdAt ? new Date((x as any).createdAt).getTime() : 0;
      case 'updatedAt':
        return (x as any).updatedAt ? new Date((x as any).updatedAt).getTime() : 0;
      case 'updatedBy':
        return ((x as any).updatedBy ?? '').toLowerCase();
    }
  }
  ...
  setShowSubThemesCount(v: boolean) {
    if (!v && this.optionalColumnsShown <= 1) return;
    this.showSubThemesCount = v;
    this.persistColumnPrefs();
    this.ensureValidSort();
  }
  ...
  private ensureValidSort() {
    const hidden =
      ...
      (this.sortKey === 'subThemesCount' && !this.showSubThemesCount) ||
    ...
    if (hidden) {
      this.sortKey = this.showName ? 'name'
        ...
        : this.showSubThemesCount ? 'subThemesCount'
    ...
    try {
      localStorage.setItem(
        ...
        JSON.stringify({
          ...
          showSubThemesCount: this.showSubThemesCount,
  ...
  private restoreColumnPrefs() {
    try {
      ...
      if (typeof obj.showSubThemesCount === 'boolean') this.showSubThemesCount = obj.showSubThemesCount;
  ...
  tableColspan(): number {
    ...
    if (this.showSubThemesCount) cols++;
